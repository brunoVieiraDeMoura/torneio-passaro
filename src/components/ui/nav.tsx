...
const userLinks = [
  ...
  {
    href: '/meus-passarinhos', label: 'Pássaros',
    icon: (active: boolean) => (
      <svg width="20" height="20" viewBox="0 0 22 16" fill="none">
        <path d="M1 8 Q5.5 1 11 8 Q16.5 15 21 8" stroke={active ? '#0D8F41' : '#9CA3AF'} strokeWidth="2.5" strokeLinecap="round"/>
      </svg>
    ),
  },
  {
...
const clubLinks = [
  {
    href: '/clube/dashboard', label: 'Início',
    icon: (active: boolean) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? '#0D8F41' : '#9CA3AF'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function Nav({ role, isLoggedIn = true }: { role: 'user' | 'club'; isLoggedIn?: boolean }) {
  const pathname = usePathname()
  const guestMode = role === 'user' && !isLoggedIn
  const links = role === 'club'
    ? clubLinks
    : guestMode
      ? [homeLink, userLinks[0], userLinks[1]]
      : userLinks
